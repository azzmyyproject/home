...
const numberOfParticles = 60; // Jumlah titik

class Particle {
    constructor() {
        this.x = Math.random() * canvas.width;
        this.y = Math.random() * canvas.height;
        this.size = Math.random() * 2 + 0.5; // Ukuran titik kecil (0.5px - 2.5px)
        this.speedX = (Math.random() - 0.5) * 0.5; // Kecepatan lambat biar smooth
        this.speedY = (Math.random() - 0.5) * 0.5;
        this.opacity = Math.random() * 0.5 + 0.2; // Transparansi acak
    }
    ...
    update() {
        ...
        // Jika keluar layar, munculkan lagi di sisi berlawanan
    ...
    draw() {
        ctx.fillStyle = `rgba(0, 255, 100, ${this.opacity})`; // Warna hijau neon transparan
        ctx.shadowBlur = 10; // Efek glow pada partikel
...
function animate() {
    // Clear canvas dengan sedikit efek trail (jejak halus)
    ctx.fillStyle = 'rgba(10, 10, 10, 0.1)'; 
    ctx.shadowBlur = 0; // Reset shadow saat clear
